function [I1,V1,I2,V2,t]=smu_internal_sweep(smu, Vgs_list, vDS, DS_comp, GS_comp, ...
    axes, curve_id, curve_color,x_var, y_var)
...
try
    ...
    %sendCommandWithCheck(smu, '*RST');
    %pause(0.2);  % Brief pause after reset
    ...
    % Calculate appropriate wait time based on NPLC and number of points
    % 1 PLC is ~16.7ms at 60 Hz power, adjust if using 50 Hz
    waitTime = numel(sweep_voltages) * 0.0167 * 1.5;  % 50% overhead
    pause(waitTime);  % Dynamic wait based on measurement time
    
    % Check if operation is complete
    complete = '0';
    timeoutStart = tic;
    while strcmp(complete, '0') && toc(timeoutStart) < visaTimeout
        complete = query(smu, '*OPC?');
        pause(0.05);
    end
